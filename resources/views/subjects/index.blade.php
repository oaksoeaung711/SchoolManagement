...
@section('search')
<x-search name="SearchSubject" value="{{ request('SearchSubject') }}" />
@endsection

@section('content')
<div class="">
    <h4 class="my-5">Subjects</h4>
    ...
    @if (Auth::user()->roles->contains(1) || Auth::user()->roles->contains(2) || Auth::user()->roles->contains(3))
        <div class="w-full md:w-1/2 flex justify-end my-5">
            <a href="{{ route('subjects.create') }}" class="w-fit btn-dark">Create</a>
        </div>
    @endif
                        ...
                        @if (Auth::user()->roles->contains(1) || Auth::user()->roles->contains(2) || Auth::user()->roles->contains(3))
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="hidden group-hover/list:flex items-center gap-5">
                                    ...
                                    <form id="formdelete-{{ $subject->id }}" action="{{ route('subjects.destroy',$subject->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </div>
