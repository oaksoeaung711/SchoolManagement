...
@section('content')
<div class="">
    <h4 class="my-5">Edit Teacher</h4>

    <div class="w-full md:w-1/3 mt-10">
        <form action="{{ route('teachers.update',$teacher->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <x-input type="text" name="name" icon="font" placeholder="Tr. Kyaw Kyaw" label="Name" value="{{ $teacher->name }}" />
            <x-input type="email" name="email" icon="envelope" placeholder="kyawkyaw@kbtc.edu.mm" label="Email" value="{{ $teacher->email }}" />
            <x-input type="phone" name="phone" icon="phone" label="Phone" value="{{ $teacher->phone }}" />
            <div class="border-2 border-gray-300 bg-slate-100 rounded-xl p-6 relative @error('sign') border-red-500 @enderror">
                ...
                <div class="text-center">
                    <img id="preview" src="{{ !empty($teacher->sign) ? asset("storage/signs/".$teacher->sign) : asset('asset/imgs/backgrounds/upload.svg') }}" class="w-40 h-40 mx-auto" />
            ...
            <div class="flex mt-10 gap-3">
                ...
                <button type="submit" class="btn-dark w-full" >Update</button>
            </div>
        </form>
    </div>
</div>
